...
class WideResNet(nn.Module):
    ...
    def forward(self, x, withres=True, withlatent=False):
        out = self.conv1(x)
        out = self.block1(out)
        out = self.block2(out)
        out = self.block3(out)
        ...
        if withlatent:
            ...
            out_[:, :self.fc.out_features] = out[:, :self.fc.out_features] + out_fin * 0
            out_[:, self.fc.out_features:] = out[:, self.fc.out_features:]
            res.append(out_)
